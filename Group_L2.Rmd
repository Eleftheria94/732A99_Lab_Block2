...
#conf_pred_model
test_error_NSC <- sum(diag(conf_pred_model)) / sum(conf_pred_model)
test_error_NSC
...
test_error_EN <- sum(diag(conf_elastic_net)) / sum(conf_elastic_net)

# Support Vector Machine with "vanilladot" kernel

SVM  <- ksvm(x = t(x_train), y = as.factor(y_train),  kernel = "vanilladot", scale = FALSE) 
                
SVM_pred <- predict(SVM, newdata = t(x_test))

# Calculate test error
conf_SVM <- table(y_test, SVM_pred)
test_error_SVM <- sum(diag(conf_SVM)) / sum(conf_SVM)
...
The model with the lowest misclassification error rate is the Nearest Shrunken Centroid one between the 3 methods. But in order to choose which model performed best we would also need to look at the number of features that were selected. Nearest Shrunken Centroid and SVM have the same and the highest amount of variables selected which makes them inefficient as they render complex models. Therefore, our choice is automatically limited to NSC and Elastic Net. Having taken both criteria into consideration, we would choose Elastic Net because, even though the test error is not the lowest, the difference between 144 and 30 variables is significant.
...
legend("topleft", legend = c("Rejected Features", "Accepted Features"), col = c("red", "blue"), lwd = 1, cex = 0.8)
